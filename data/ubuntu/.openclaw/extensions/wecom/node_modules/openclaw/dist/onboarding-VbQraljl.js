...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	...
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DYEtgk1X.js").then((n) => n.t);
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-HaS1kfvg.js").then((n) => n.n);
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-wXGdYsUg.js");
