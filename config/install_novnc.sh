...
wget -O /etc/apt/sources.list.d/TurboVNC.list https://ghfast.top/https://raw.githubusercontent.com/TurboVNC/repo/main/TurboVNC.list
apt-get update
apt-get install -y turbovnc
rm /etc/apt/sources.list.d/TurboVNC.list
# install novnc
curl -fsSL "https://ghfast.top/https://github.com/novnc/noVNC/archive/v${NOVNC_VERSION}.tar.gz" | tar -xzf - -C /opt
mv -f "/opt/noVNC-${NOVNC_VERSION}" /opt/noVNC
ln -snf /opt/noVNC/vnc.html /opt/noVNC/index.html
git clone "https://ghfast.top/https://github.com/novnc/websockify.git" /opt/noVNC/utils/websockify
# config turbovnc
